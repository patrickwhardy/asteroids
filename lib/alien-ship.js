...
AlienShip.prototype.fireWeapon = function() {
  this.clearBullets()
  return this.bullets.push(new Bullet({ x: this.shipCenter.x, y: this.shipCenter.y, slope: this.findOpponent()}, this.context))
}
...
AlienShip.prototype.findOpponent = function() {
  var y = (this.shipCenter.y - this.opponent.y) * .001
  var x = (this.shipCenter.x - this.opponent.x) * .001
  return {x: x, y: y}
}
