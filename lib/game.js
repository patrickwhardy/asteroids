...
Game.prototype.createAsteroid = function(asteroidCount, level, asteroid) {
  if (asteroid) {
...
Game.prototype.checkBulletCollision = function() {
  ...
  if (thisGame.ship.bullets.length > 0) {
    thisGame.asteroids.forEach(function(asteroid, index){
      thisGame.ship.bullets.forEach(function(bullet){
        ...
        (bullet.center.y < asteroid.center.y + asteroid.radius - 5)) {
          ...
          thisGame.asteroidsToRemove.push(index)
        }
      })
    })
  }
...
Game.prototype.explodeAsteroid = function(asteroid) {
  if (asteroid.radius > 60) {
    var ass = this.createAsteroid(this.asteroids.count, this.level, new Asteroid(this.context, asteroid.center, (asteroid.radius / 3)))
    // debugger;
    // this.createAsteroid(this.asteroids.count, this.level, new Asteroid(this.context, asteroid.center, (asteroid.radius / 3)))
    // this.createAsteroid(this.asteroids.count, this.level, new Asteroid(this.context, asteroid.center, (asteroid.radius / 3)))
  }
}
